...
>
	...
	>
		<header
			...
			class="flex items-center justify-between gap-3 border-blue-100 bg-blue-50/80 px-4 py-3 dark:border-gray-800 dark:bg-gray-850"
		>
			...
			<div class="flex shrink-0 items-center gap-1">
				<button
					...
					aria-controls="agui-interaction-card-content"
					aria-label={collapsed ? '展开选择框' : '缩小选择框'}
					title={collapsed ? '展开选择框' : '缩小选择框'}
					on:click={() => (collapsed = !collapsed)}
				>
					<LucideIcon
						name={collapsed ? 'chevron-up' : 'minimize-2'}
						className="size-4"
						strokeWidth="2"
					/>
				</button>
		...
		{#if !collapsed}
			<div
				id="agui-interaction-card-content"
